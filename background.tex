...
\jmh{The following sentence is implicit in the definition of cartesian product and selection.} We further assume that the join operation is commutative, left associative, and right associative.
\jmh{The ``very straight-forward'' statement is a bit unnerving, especially after the assertion about commutativity and associativity, which outer joins don't promise. Maybe a forward ref here to an ``extensions'' discussion later in the paper, where you address expanding the search space to handle outer joins.a}
